...
export function useDijkstra() {
  ...
  const findShortestPath = async () => {
    ...
    while (unvisited.size > 0 && current) {
      ...
      console.log(
        `Processing node ${current} with distance ${currentDistance}`
      );
      ...
      for (const { node: neighbor, weight } of neighbors) {
        if (unvisited.has(neighbor)) {
          ...
          if (newDistance < currentDistance) {
            console.log(
              `Updating distance to ${neighbor}: ${currentDistance} -> ${newDistance}`
            );
    ...
    if (distances.get(endNodeId) !== Infinity) {
      ...
      steps.push({
        ...
        path: finalPath,
        message: `Found shortest path with distance ${distances.get(
          endNodeId
        )}`,
      });
    }
  ...
  const loadExample = (index: number) => {
    ...
    example.nodes.forEach((node) => {
      initialDistances.set(
        node.id,
        node.id === example.startNode ? 0 : Infinity
      );
    });
